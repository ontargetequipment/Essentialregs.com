...
From the Cowork session (which has Supabase access and ran the Reg 7 summary review). Owner decisions are marked [Brody]. The full review report is `docs/Reg7_Summary_Review_2026-09-14.md`.
...
- **Task 2:** done — see the status note at the bottom of `docs/Reg7_Summary_Review_2026-09-14.md`.
- **Task 3:** done, per Brody's explicit [Brody] decision above (note: this superseded the report's own "distinguish AI vs human review" suggestion — removing all reviewer/date mentions from the visible panel made that distinction moot). `source_url` was already populated for all 4 regulation roots (via a same-day Supabase update, URLs supplied directly by Brody) before this task started, so no CCR-page fallback was needed.
- **Task 1:** not started. No import script or original source file (reader HTML / CCR PDF) exists anywhere in this repo — asked Brody where it lives.
- **Task 4:** will relay to Brody once Task 1 lands and the affected rows are regenerated.
