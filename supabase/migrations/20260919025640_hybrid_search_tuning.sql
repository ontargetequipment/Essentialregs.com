-- Load pgvector in this session first so its hnsw.* GUCs are registered
-- (otherwise the function SET clauses fail with "permission denied to set parameter").
...
begin
  ...
  with hits as (
    ...
    limit case when reg_filter is null and jurisdiction_filter is null then match_count * 6 else match_count * 2 end
  ),
  best as (
...
begin
  ...
  sem_hits as (
    ...
    where (reg_filter is null or e.reg_key = any (reg_filter))
      ...
      and (include_basis or not e.is_basis)
    order by e.embedding <=> query_embedding
    limit case when reg_filter is null and jurisdiction_filter is null then pool * 2 else pool end
  ),
  sem as (
