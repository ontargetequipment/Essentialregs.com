-- Reconciled 2026-09-24. Source: docs/imports/2026-09-21/RUN_ME_neighbors_rpc_2026-09-21.sql
-- (left in place there as part of the Batch 6 import record). Pasted into the
-- Supabase SQL editor and never entered supabase_migrations.schema_migrations;
-- this file puts it on the record. It was never in supabase/migrations/, but
-- production carries its four-argument recompute_provision_neighbors and
-- 011_security_grants.sql (next in sequence) revokes on exactly that signature,
-- so a replay without this file would fail at 011.
--
-- When it ran: 2026-09-21 (011_security_grants.sql: the superseded three-argument
-- signature was "dropped by hand on 2026-09-21"; the fix that drops it was written
-- the same day). This version number is the UTC time the file first entered git
-- (1ec567e, 2026-09-21 18:51:35 UTC). It sorts after 20260919041700 and before
-- 20260922030526_security_grants, which is the order that matters.
--
-- Production evidence: recompute_provision_neighbors(text[], integer, integer,
-- integer) exists, its body matches this file modulo whitespace (production has
-- CRLF line endings from the paste), the three-argument overload is gone, and
-- EXECUTE is held by service_role only.
--
-- Nothing below the next line was changed. The leading DROP makes it safe on a
-- fresh database (where 20260918040936 has just created the three-argument version).
-- ---------------------------------------------------------------------------
